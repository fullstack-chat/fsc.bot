...
client.on("ready", async () => {
  ...
  let guild = client.guilds.cache.find(
    (guild) => guild.id == "553773331674038282"
  );
  let general = guild.channels.cache.find(
    (channel) => channel.id == "553773331674038284"
  );
...
client.on("error", (e) => {
  ...
  let logChannel = guild.channels.cache.find(
    (channel) => channel.id == "738860914723258429"
  );
  logChannel.send(e);
});
...
client.on("guildMemberAdd", async (member) => {
  console.log(
    `**${member.user.username}** just joined **${member.guild.name}**!`
  );
  await security.sendModBroadcast(
    member.guild,
    `**${member.user.username}** just joined **${member.guild.name}**!`
  );
});
...
client.on("message", async (message) => {
  ...
  if (message.mentions.users.has("739144597934178367")) {
    let phrase = message.content
      .replace("<@!739144597934178367> ", "")
      .toLowerCase();
    if (phrase.includes("tell me a joke")) {
      let joke = await getDadJoke();
      console.log(joke);
      return message.channel.reply(joke);
    }
  }
